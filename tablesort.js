    ...
    setup: function(table,tableSortContainer) {
      ...
      function bindTh(th, isNum) {
        th.on('click',function () {
          ...
          if (isNum) {
            sortArr = sortArr.sort((a, b) => a.split('_')[0] - b.split('_')[0]);
          } else {
            sortArr = sortArr.sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
          }
          if (sortOrder === 'asc') {
      ...
      table.find('th').each(function () {
        ...
        if ($(this).hasClass('table-sort')) {
          format(th);
          bindTh(th, false);
        } else if ($(this).hasClass('table-sort-num')) {
          format(th);
          bindTh(th, true);
        }
      });
